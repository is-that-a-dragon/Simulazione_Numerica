...
void Input(void)
{
 ...
 //Read input informations
  ...
  //Tail correction: you only need rcut
  ...
  if(iNVET){
    ...
    cout << "The program performs Molecular Dynamics" << endl;
    cout << "Timestep = " << delta << endl;
    }
 ...
 //Prepare arrays for measurements
  ...
  //measurement of g(r)
  igofr = 6; //index for g(r)
  nbins = 500;
  n_props = n_props + nbins;  //metto i 100 bin nel walker
...
void Measure() //Properties measurement
{
    ...
    for (int j=i+1; j<npart; ++j)
    {
     // distance i-j in pbc
      ...
      if(dr < rcut)
      {
        ...
        w += 48.0*(1.0/pow(dr,12) - 0.5/pow(dr,6));
      }
    }          
  }
  ...
  walker[iw] = rho*walker[it] + ( 1./(3.*vol) )*( w/(double)npart ); //Pressure
...
{    
  if(equilibrazione && iblk%1000 == 0){
    cout << "Block number " << iblk << endl;
    cout << "Acceptance rate " << accepted/attempted << endl << endl;
    cout << "----------------------------" << endl << endl;
  }
  else if(equilibrazione == false){
    cout << "Block number " << iblk << endl;
    cout << "Acceptance rate " << accepted/attempted << endl << endl;
    cout << "----------------------------" << endl << endl;    
  }

  stima_pot = ( blk_av[iv] + vtail )/blk_norm/(double)npart; //Potential energy
  ...
  stima_pres = (blk_av[iw] + ptail) /blk_norm; //Pressure
  ...
  if(equilibrazione){
    ofstream Equil;
    Equil.open("correlation_folder/gas_correlation.dat", ios::app);
    Equil << iblk << "," << stima_pot << endl;
    // block (or step) number --- ith potential energy

    Equil.close();
  }
  */
  if(equilibrazione == false){
    ofstream Epot, Press, Gofr, Gave;

    //Potential Energy
    Epot.open("liquid/epot_NVE.dat",ios::app);
    Epot << iblk <<  "," << stima_pot << "," << glob_av[iv]/(double)iblk << "," << err_pot << endl;

    //Pressure
    Press.open("liquid/pres_NVE.dat", ios::app);
    Press << iblk <<  "," << stima_pres << "," << glob_av[iw]/(double)iblk << "," << err_pres << endl;
    
    Gofr.open("liquid/gofr_NVE_perblock.dat",ios::app);
    Gave.open("liquid/gofr_NVE.dat",ios::app);

    double delta_V; //volume sfera r,r+dr

    Gofr << endl << iblk << endl;
    for (int i = igofr; i < igofr+nbins ; i++ ){ //cycle on hystogram walkers
        delta_V = 4.*M_PI*( pow(bin_size*(double)(i+1.), 3. ) - pow( bin_size*(double)(i), 3.))/3.;
        glob_av[i] +=  blk_av[i]/blk_norm/(rho * delta_V * (double)npart);
        glob_av2[i] += pow( blk_av[i]/blk_norm/(rho * delta_V * (double)npart) , 2. );
...
void ConfFinal(void)
{
  ...
  WriteVelocity.close();
  if(equilibrazione)
    cout << "\033[1m\033[37m" << "\nEquilibration completed.\nYou can now restart the program with the correct number of blocks, steps and temperature\n\n" << "\033[0m";
